...
export default function App(){
  ...
  // ================= COMBINE SCORES =================
  function combineScores(data){

    const grouped = {};

    data.forEach(e=>{
      const key = e.car + "_" + e.eventName;
      ...
      grouped[key].judges += 1;
      grouped[key].deductions = [
        ...new Set([...grouped[key].deductions, ...(e.deductions || [])])
      ];
    });
  ...
  function Leaderboard({ type }){
    ...
    let list = combineScores(entries.filter(e=>e.eventName===eventName));
          ...
          {classes.map(cls=>(
            <div key={cls}>
              ...
              {list.filter(e=>e.carClass===cls)
                ...
                .map((e,i)=>(
                  <div key={i}>
                    #{i+1} | {e.car} | {e.total} ({e.judges} judges)
                  </div>
              ))}
            </div>
          ))}
